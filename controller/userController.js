...
const login = async (req, res) => {
    ...
    try {
        ...
        const jwtToken = jwt.sign({ id: userWithEmail.user_id, email: userWithEmail.email }, "secrettoken")
...
const register = async (req, res) => {
    const { firstName, lastName, dob, email, userName, password } =
        req.body;
    const hashPass = bcrypt.hashSync(password, 10)
    console.log(hashPass)
    const isAlreadyUser = await User.findOne({ where: { email } }).catch(err => {
        console.log("ERROR", err)
    })
    if (isAlreadyUser) {
        return res.json({ status: "error", error: "Email is already exists" })
    }
    const newUser = new User({ firstName, lastName, dob, email, userName, password: hashPass })
