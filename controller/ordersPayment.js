...
const storage = multer.memoryStorage(); // Use memory storage to work with buffers
const BLOB_READ_WRITE_TOKEN =
  "vercel_blob_rw_BIYtuWu7t29bWdq5_TuQx9tnGXHepVL4pUVmqMNN3GE2QFl";
...
const createOrderWithDetailsAndPayment = async (req, res) => {
  try {
    ...
    upload(req, res, async (err) => {
      const file = req.file;
      const uploadResult = await put(file.originalname, file.buffer, {
        access: "public",
      ...
      // Contains the uploaded image file details

      // Create the payment and associate it with the order
      const payment_data = {
        // payment_id: uuid2,
        total_price: req.body.total_price,
        payment_picture: uploadResult.url,
      ...
      res
        .status(201)
        .json({
          message: "payment success waiting admin confirm",
          data: payment_result,
        });
    });
  } catch (err) {
...
module.exports = {
  createOrderWithDetailsAndPayment,
  getOrders,
  addExchangeRate,
};
